  ...
  return (
    ...
    >
      ...
      <div id="country-sidebar-panel" className="lg:col-span-4 space-y-4">
        ...
        >
          {filteredCountries.map((c) => (
            ...
            >
              <CountryFlag
                countryNameOrCode={c.code}
                className="w-8 h-5.5 rounded-sm object-cover shadow-sm border border-neutral-800/20 shrink-0"
              />
              <div className="flex-1 min-w-0">
                <p className="font-semibold text-sm truncate">{c.name}</p>
                <p
                  className={`text-xs truncate ${selectedCountry.code === c.code ? "text-neutral-700" : "text-neutral-500"}`}
                >
          ...
          {filteredCountries.length === 0 && (
            <p className="text-neutral-600 text-sm py-8 text-center font-medium">
              Хайлттай тохирох улс олдсонгүй.
            </p>
          )}
        </div>
      </div>
      ...
      <div
        ...
        className="lg:col-span-8 bg-neutral-900/30 border border-neutral-800 rounded-3xl p-6 md:p-8 backdrop-blur-md relative overflow-hidden"
      >
        ...
        <div className="absolute top-4 right-4 opacity-[0.04] w-64 h-44 select-none pointer-events-none overflow-hidden rounded-2xl">
          <CountryFlag
            countryNameOrCode={selectedCountry.code}
        ...
        <div className="flex items-center gap-4 mb-6">
          <div className="p-1.5 bg-neutral-900 border border-neutral-850 rounded-2xl shadow-md shrink-0 flex items-center justify-center">
            <CountryFlag
              countryNameOrCode={selectedCountry.code}
              className="w-14 h-9.5 rounded-xl object-cover"
            />
          </div>
          <div>
            <div className="flex items-center gap-2">
              <span className="text-xs bg-neutral-800 text-neutral-300 px-2.5 py-0.5 rounded-full font-mono uppercase tracking-wider">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
          <div
            id="info-living-cost"
            className="p-4 bg-neutral-950/40 border border-neutral-850 rounded-2xl flex items-start gap-3.5"
          >
            ...
            <div>
              <p className="text-[11px] font-semibold text-neutral-500 uppercase tracking-wider">
                Амьжиргааны дундаж зардал
              </p>
              <p className="text-sm font-semibold text-white mt-1">
                {selectedCountry.averageLivingCost}
              </p>
            </div>
          </div>
          <div
            id="info-tuition-cost"
            className="p-4 bg-neutral-950/40 border border-neutral-850 rounded-2xl flex items-start gap-3.5"
          >
            <div className="p-2.5 bg-neutral-900 border border-neutral-800 rounded-xl text-neutral-400">
            ...
            <div>
              <p className="text-[11px] font-semibold text-neutral-500 uppercase tracking-wider">
                Сургалтын дундаж төлбөр
              </p>
              <p className="text-sm font-semibold text-white mt-1">
                {selectedCountry.averageTuitionCost}
              </p>
            </div>
          </div>
        </div>
        ...
        {selectedCountry.code !== "Mongolia" &&
          selectedCountry.exchangeRateMNT && (
            <div
              id="currency-calculator"
              className="mt-8 p-4 bg-amber-500/5 border border-amber-500/10 rounded-2xl"
            >
                ...
                <div className="relative w-full sm:w-1/2">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      const resultMnt =
                        val * (selectedCountry.exchangeRateMNT || 1);
                      const el = document.getElementById(
                        "mnt-converted-output",
                      );
                      if (el) {
                        el.innerText =
                          resultMnt.toLocaleString(undefined, {
              ...
              <div className="mt-2 text-[9px] text-neutral-500">
                Ханшийн мэдээлэл: 1 {selectedCountry.code} нэгж ={" "}
                {selectedCountry.exchangeRateMNT?.toLocaleString()} ₮
              </div>
            </div>
          )}
        ...
        <div className="mt-8 pt-6 border-t border-neutral-800/80 grid grid-cols-2 md:grid-cols-3 gap-4 text-xs">
          ...
          <div className="col-span-2 md:col-span-1 border-t md:border-t-0 pt-3 md:pt-0 border-neutral-800">
            ...
            <span className="text-white font-bold text-sm mt-0.5 block font-mono">
              {selectedCountry.exchangeRateText || "1.00 ₮"}
            </span>
          </div>
        </div>
      </div>
