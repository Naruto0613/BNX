...
export default function AcademicProfilePDFModal({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex flex-col items-center justify-start overflow-y-auto p-4 md:p-8">
      ...
      <div className="print:hidden w-full max-w-4xl bg-neutral-900 border border-neutral-800 p-4 rounded-2xl mb-6 flex items-center justify-between gap-4 sticky top-2 z-50 shadow-2xl">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={handlePrint}
            className="px-5 py-2.5 bg-white hover:bg-neutral-200 text-black font-extrabold text-xs rounded-xl shadow-lg transition active:scale-95 flex items-center gap-2"
          >
            <Printer className="w-4 h-4" />
            PDF-ээр Хадгалах / Хэвлэх
          </button>
          <button
            onClick={onClose}
            className="p-2.5 text-neutral-400 hover:text-white bg-neutral-800 hover:bg-neutral-700 rounded-xl transition"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
