...
export default function EssayHelper({
  ...
  const [tab, setTab] = useState<
    "grammar" | "ielts" | "structure" | "suggestions"
  >("grammar");
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "AI essay analyzer API fallback to client-side heuristics:",
          e,
        );
      }

      if (!analysis) {
        // High quality heuristic feedback
        const words = content.trim().split(/\s+/).length;
        const paragraphs = content
          .split(/\n\s*\n/)
          .filter((p) => p.trim().length > 0).length;
        const estScore =
          words > 450 ? 7.5 : words > 300 ? 7.0 : words > 150 ? 6.5 : 6.0;

        analysis = {
          scoreEstimate: estScore,
          grammarCorrections: [
            {
              original: "Ерөнхий дүрмийн зөвлөмж",
              corrected: "Academic Style",
              explanation:
                "Өгүүлбэрийн эхлэлүүдээ олон янзаар баяжуулж, академик үг хэллэг түлхүү ашиглахыг зөвлөж байна.",
            },
          ],
          ieltsFeedback: `Нийт ${words} үгтэй, ${paragraphs} параграф бүхий зохиомжтой байна. Үгийн баялаг болон найруулгын хувьд бакалаврын түвшний эссэнд ойролцоогоор Band ${estScore} түвшний үнэлгээ авч чадахаар байна.`,
          structureAnalysis:
            paragraphs >= 4
              ? "Бүтэц маш сайн: Оршил, үндсэн тайлбар параграфууд болон дүгнэлт хэсэг тодорхой ялгарч байна."
              : "Бүтцийн зөвлөгөө: Эссэгээ Оршил (Intro), 2-3 Үндсэн бие (Body), болон Дүгнэлт (Conclusion) болгон параграф тус бүрт хуваах нь уншихад илүү цэгцтэй болгоно.",
          suggestions: [
            "Хувийн туршлага, тулгарсан сорилт болон түүнээс сурсан сургамжаа тодорхой баримтаар жишээ татах",
            "Сонгосон мэргэжлээрээ тухайн их сургуульд яагаад суралцах хүсэлтэй байгаагаа шууд холбох",
            "Идэвхтэй хэв маяг (Active Voice) ашиглан итгэл үнэмшилтэй бичих",
          ],
        };
      }
      ...
      const updatedEssay: Essay = {
        ...
        userId: activeEssay ? activeEssay.userId : uid || "current",
        title: title || "Admissions Essay Draft",
        content,
        scoreEstimate: analysis.scoreEstimate,
        grammarCorrections: analysis.grammarCorrections || [],
        ieltsFeedback: analysis.ieltsFeedback || "Эссэний шинжилгээ амжилттай.",
        structureAnalysis:
          analysis.structureAnalysis || "Зохион байгуулалт шалгагдсан.",
      ...
      console.error("Essay analysis note:", err);
    } finally {
  ...
  const handleDeleteDraft = async (eId: string, e: React.MouseEvent) => {
    ...
    if (
      window.confirm(
        "Уг эссэний ноорог болон шинжилгээг устгах уу? This draft and its feedack will be cleared.",
      )
    ) {
      try {
  ...
  return (
    <div
      id="essay-helper-root"
      className="grid grid-cols-1 lg:grid-cols-12 gap-8 pb-16"
    >
      ...
      <div id="essay-sidebar" className="lg:col-span-4 space-y-4">
        <div className="bg-neutral-900/30 border border-neutral-800 p-4 rounded-2xl">
          ...
          >
                ...
                <button
                  ...
                  className={`p-1 rounded shrink-0 ${
                    activeEssay?.id === e.id
                      ...
                      : "hover:bg-neutral-950 text-neutral-500 hover:text-rose-400"
                  }`}
                  title="Устгах"
                >
                  &times;
            ...
            {essays.length === 0 && (
              <p className="text-neutral-650 text-xs py-10 text-center">
                Хадгалсан ноорог алга. Ноорог бичээд AI шинжилгээ ажиллуулаарай.
              </p>
            )}
          </div>
        </div>
        ...
        <div
          id="essay-info-card"
          className="p-4 bg-neutral-900/10 border border-neutral-850 rounded-xl space-y-2"
        >
          ...
          <p className="text-[11px] text-neutral-400 leading-relaxed">
            Сургуулиудын хувийн тодорхойлолт эссэнд сурлагын дүн биш, таны хэн
            болох, туулсан амьдрал, шийдлийг сонирхоно. IELTS Feedback хэсгээс
            үгсийн баялгаа баталгаажуулаарай.
          </p>
        </div>
      </div>
      ...
      <div id="essay-editor-wrapper" className="lg:col-span-8 space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="flex justify-between text-[11px] text-neutral-500 mt-1">
                <span>
                  Үгийн тоо: {content ? content.trim().split(/\s+/).length : 0}
                </span>
        ...
        {activeEssay && activeEssay.scoreEstimate && (
          <div
            id="essay-analysis-results"
            className="bg-neutral-900/50 border border-neutral-800 rounded-3xl p-6 md:p-8 space-y-6"
          >
            {/* Score header */}
            <div className="flex items-center justify-between border-b border-neutral-800/80 pb-4">
              <div>
                ...
                <p className="text-xs text-neutral-550 mt-0.5">
                  Шинжилсэн сэдэв: {activeEssay.title}
                </p>
              </div>
              <div className="bg-white text-black px-4 py-2 rounded-2xl text-center shadow-lg">
                <span className="text-[10px] uppercase font-bold tracking-widest text-neutral-550 block">
                  Band Score
                </span>
                <span className="text-md font-extrabold">
                  {activeEssay.scoreEstimate}
                </span>
              </div>
            </div>
            ...
            <div
              id="essay-tab-content"
              className="bg-neutral-950/30 border border-neutral-850 rounded-2xl p-5 text-neutral-300 text-xs md:text-sm leading-relaxed whitespace-pre-line font-sans"
            >
              {tab === "grammar" && (
                <div>
                  ...
                  <p className="text-neutral-300">
                    {activeEssay.grammarCorrections}
                  </p>
                </div>
              )}
              {tab === "ielts" && (
