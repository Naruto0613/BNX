  ...
  const getAIScholarships = async () => {
    ...
    try {
      ...
      if (!resp.ok) {
        ...
        throw new Error(errData.error || "Server returned error during scholarship curation.");
      }
        ...
        <div className="max-w-2xl">
          ...
          {!profile.name && (
            <p className="text-[10px] text-amber-400/80 mt-1 flex items-center gap-1 font-mono">
              <AlertCircle className="w-3.5 h-3.5" /> Эхлээд &lsquo;Академик Профайл&rsquo; табт орж голч мэдээллээ бүртгүүлнэ үү.
            </p>
          )}
        </div>
      </div>
