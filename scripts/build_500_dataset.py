...
# Comprehensive JSON dataset generator of 500 accredited real universities across 55 countries
# Formatted for Firebase Firestore import and BNX platform schema

universities = []

def uni(uid, name, short_name, country, city, continent, qs, utype, website,
        tuition_intl, tuition_dom, currency, living_cost, app_fee, ielts, toefl, duolingo, gpa,
        majors, desc, founded, pop, intl_pop, accept_rate, campus="Urban", climate="Temperate", emp_rate=92.0):
    ...
    universities.append({
        ...
        "description": desc or f"{name} is an internationally accredited institution located in {city}, {country}.",
        "campusType": campus,
        "climate": climate,
...
# Master Dataset List of 500 Accredited Universities across 55 Countries
raw_data = [
    # 1. UNITED STATES (60)
...
for row in raw_data:
    uni(row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7], row[8], row[9], row[10], row[11], row[12], row[13], row[14], row[15], row[16], row[17], row[18], row[19], row[20], row[21], row[22], row[23])

print("Currently added entries:", len(universities))
